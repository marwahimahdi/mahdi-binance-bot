...
- Auto-Scan Mode (Top N by 24h quote volume) — default 30
...
- Dynamic leverage/size: 5x/10x and 5%/6% (total cap 40%, max open=6)
- 3 TPs: 0.35% / 0.7% / 1.2% (strong: 0.5% / 1.0% / 1.8%)
- After TP1: SL -> Breakeven | After TP2: lock profit (+0.20%, strong +0.30%)
- Kill-switch (daily realized PnL <= -5% balance)
- Watchdog: warn after 10 min no-activity + reminder after 30 min
- Force ISOLATED margin; gracefully warn if blocked by open orders
- Telegram heartbeat + detailed trade lifecycle messages
"""
...
def fetch_valid_perpetual_usdt():
    ...
    valid={s["symbol"] for s in data.get("symbols",[])
